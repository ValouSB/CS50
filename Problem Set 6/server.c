...
char* indexes(const char* path)
{
    char* c1 = malloc(sizeof(char)*(11 + strlen(path)));
    char* c2 = malloc(sizeof(char)*(12 + strlen(path)));
    char* c3 = malloc(sizeof(char)*(11 + strlen(path)));
    if (c1 == NULL || c2 == NULL || c3 == NULL) return NULL;
    strcpy(c1, path);
    strcpy(c2, path);
    strcpy(c3, path);
    strcat(c1, "/index.php");
    strcat(c2, "/index.html");
    strcat(c3, "/index.htm");
    if (access(c1, F_OK) != -1) return c1;
    else free(c1);
    if (access(c2, F_OK) != -1) return c2;
    else free(c2);
    if (access(c3, F_OK) != -1) return c3;
    else free (c3);
    return NULL;
}
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    for (int c = fgetc(file); c != EOF; c = fgetc(file))
    {
        if (count + 1 > capacity)
        {
            ...
            if (realloc(buffer, capacity * sizeof(char)) == NULL)
            {
                free(buffer);
                return false;
            }
...
/**
 * Returns MIME type for supported strrchr(path, '.')s, else NULL.
 */
const char* lookup(const char* path)
{
        if (strcasecmp(strrchr(path, '.'), ".css") == 0) return "text/css";
        else if (strcasecmp(strrchr(path, '.'), ".html") == 0) return "text/html";
        else if (strcasecmp(strrchr(path, '.'), ".gif") == 0) return "image/gif";
        else if (strcasecmp(strrchr(path, '.'), ".ico") == 0) return "image/x-icon";
        else if (strcasecmp(strrchr(path, '.'), ".jpg") == 0) return "image/jpeg";
        else if (strcasecmp(strrchr(path, '.'), ".js") == 0) return "text/javascript";
        else if (strcasecmp(strrchr(path, '.'), ".php") == 0) return "text/x-php";
        else if (strcasecmp(strrchr(path, '.'), ".png") == 0) return "image/png";
        else return NULL;
}
...
{
    char temp[(strlen(line) + 1) * sizeof(char)];
    strcpy(temp, line);
    char cm[strlen(temp)* sizeof(char)];
    char ct[strlen(temp)* sizeof(char)];
    char cv[strlen(temp)* sizeof(char)];
    int tcount = 0;
    char* t = strtok(temp, " ");
    while (t != NULL)
    {
        if (tcount == 0) strcpy(cm, t);
        else if (tcount == 1) strcpy(ct, t);
        else if (tcount == 2) strcpy(cv, t);
        else
        {
            error(400);
            return false;
    ...
    else if(strchr(ct, '?') != NULL)
    {
        tcount = 0;
        char* ccount = strtok(ct, "?");
        while (ccount != NULL)
        {
            if (tcount == 0) strcpy(abs_path, ccount);
            else if(tcount == 1) strcpy(query, ccount);
            ccount = strtok(NULL, "?");
            tcount++;
        }
    }
